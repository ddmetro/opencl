...
cl_mem LoadImage(cl_context context, char *fileName, int &width, int &height)
{
    ...
    c.rgbBlue = 0xFF;
    c.rgbReserved = 0xFF;
    image = FreeImage_EnlargeCanvas(image, 1, 1, 1, 1, &c, FI_COLOR_IS_RGB_COLOR);
    FreeImage_Unload(temp1);
    ...
    width = width - 2;//adjusting for the added border 
    height = height - 2;//adjusting for the added border
    return clImage;
}
...
int main(int argc, char** argv)
{
    ...
    size_t globalWorkSize[1] =  { RoundUp(localWorkSize[0], height) };
    ...
    errNum |= clSetKernelArg(kernel, 5, sizeof(float) * ((width+2)*(localWorkSize[0]+2)*4), NULL);
    if (errNum != CL_SUCCESS)
    {
    ...
    std::cout << "Total time taken: " << duration << " milliseconds" << std::endl;
    return 0;
}
